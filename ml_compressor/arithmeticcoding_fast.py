...
class ArithmeticDecoder(ArithmeticCoderBase):
	...
	def __init__(self, statesize, bitin):
		...
		for _ in range(self.STATE_SIZE):
			read_bit = int(self.read_code_bit())
			self.code = self.code << 1 | read_bit
			code_read.append(read_bit)
...
class BitInputStream(object):
	...
	def __init__(self, inp):
		...
		## 2 modes: either input is a txt file or ndarray
		if isinstance(self.input, np.ndarray):
			assert self.input.ndim == 1, "input should be 1-d array"
			self.mode = "numpy-array"
		else:
			self.mode = "IOBase"
	...
	def read(self):
		...
		# print("file: ", (self.currentbyte >> self.numbitsremaining) & 1)
...
class BitOutputStream(object):
	...
	def __init__(self, out=None):
		if isinstance(out, io.IOBase):
			...
			raise ValueError("Output Stream should either be None (result in numpy array) or IOBase")
	...
	def renew(self):
		...
		self.num_zero_padding = 0
		self.compressed_bits = []

	@property
	def compressed_bits_length(self):
		## including the possible zero-paddings for X*8 (bytes)
		if isinstance(self.compressed_bits, list):
			return len(self.compressed_bits)
		else:
			return self.compressed_bits.shape[0]
	...
	def write(self, b):
		...
		if self.numbitsfilled == 8:
			...
			self.output.write(towrite) if self.mode != "numpy-array" else None
			self.compressed_bits.extend(format(ord(towrite), '08b'))
			self.currentbyte = 0
			self.numbitsfilled = 0
	...
	def close(self):
		...
		while self.numbitsfilled != 0:
			self.num_zero_padding += 1
			self.write(0)
		self.compressed_bits = np.array(self.compressed_bits).astype(int)
		self.output.close() if self.mode != "numpy-array" else None
